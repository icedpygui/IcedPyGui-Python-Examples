...
Styling color notes:
 ...
 Of the three containers, Container, Column, and Row, the only one that has styling is the container. If you want 
 styling for the other two, place them within a Container.

If no styling is supplied, then the default theme styling will be used.  However, you can used the style_standard 
option for each widget to select a limited number of styles such as primary, success, and danger.  The theme colors 
are limited at this time but during an upcoming revision, a custom theme will be implemented.  You can however use 
any of the IpgColors to style your widgets.  There are 155 colors (https://www.w3schools.com/cssref/css_colors.php) 
to choose from or make up you  own using the rgba option.

Internally to Iced, there are 4 special color parameters used in add_styling_colors, base, strong, weak, 
and text. These colors are used to color multiple parts of the widget to show mouse hover and drag effects, 
etc.  These colors are calculated mostly using the background base color.  There are some where the theme primary 
color is used which is a predefined color for each theme color.

To keep things simple, when you want to add colors to your widgets, IPG allows you to define the colors you want by 
using terms like background_color_hovered versus background.weak as done in Iced.  In some cases if you define, 
for example, a new background and nothing more, your background color may be incorporated and used to define other 
colors for the widget using a background strong or weak color.

The program below, generates the base, strong and weak colors to assist you in selecting the colors for your widget.  
If you have another color you would like to add, just use the rgba format."""
...
colors = [IpgColor.PRIMARY, IpgColor.SECONDARY, IpgColor.SUCCESS, IpgColor.DANGER, IpgColor.WARNING,
          ...
          IpgColor.LAVENDER_BLUSH, IpgColor.LAWN_GREEN, IpgColor.LEMON_CHIFFON, IpgColor.LIGHT_BLUE,
          IpgColor.LIGHT_CORAL,
          ...
          IpgColor.MEDIUM_SEA_GREEN, IpgColor.MEDIUM_SLATE_BLUE, IpgColor.MEDIUM_SPRING_GREEN,
          IpgColor.MEDIUM_TURQUOISE,
          IpgColor.MEDIUM_VIOLET_RED, IpgColor.MIDNIGHT_BLUE, IpgColor.MINT_CREAM, IpgColor.MISTY_ROSE,
          IpgColor.MOCCASIN,
          ...
          IpgColor.PALE_VIOLET_RED, IpgColor.PAPAYA_WHIP, IpgColor.PEACH_PUFF, IpgColor.PERU, IpgColor.PINK,
          IpgColor.PLUM,
